...
sdcard_rw
[ ! "$MODDIR" ] && MODDIR=${0%/*}
MODPATH="/data/adb/modules/Optimize—data—rates"
...
if [[ "$(getprop init.svc.thermal-engine)" == "running" ]] || [[ "$(getprop init.svc.mi_thermald)" == "running" ]] || [[ "$(getprop init.svc.thermald)" == "running" ]] || [[ "$(getprop init.svc.Optimize—data—rateservice)" == "running" ]]; then
thermal=开
elif [[ "$(getprop init.svc.thermal-engine)" == "stopped" ]] || [[ "$(getprop init.svc.mi_thermald)" == "stopped" ]] || [[ "$(getprop init.svc.thermald)" == "stopped" ]] || [[ "$(getprop init.svc.Optimize—data—rateservice)" == "stopped" ]]; then
thermal=关
else
thermal=未知
fi
...
X "$date█▓▒▒░░░📱欢迎使用设备性能优化░░░▒▒▓█" "🛠️idyll_优化数据速率™@idyll™®2018🌿"
